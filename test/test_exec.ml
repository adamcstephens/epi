...
let tests ~bin =
  [
    Alcotest.test_case "fails with usage when no command given" `Quick
      (fun () ->
    ...
    Alcotest.test_case "reports not-running for default instance" `Quick
      (fun () ->
        with_state_dir (fun state_dir ->
            let result =
            ...
            assert_contains ~context:"exec not running stderr" stderr
              "not running"));
    Alcotest.test_case "reports not-running for named instance" `Quick
      (fun () ->
        with_state_dir (fun state_dir ->
