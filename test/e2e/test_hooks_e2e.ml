let contains s sub =
  ...
  let rec check i =
    i + subl <= sl && (String.sub s i subl = sub || check (i + 1))
  in
  check 0
  ...
  [
    Alcotest.test_case "all hook sources run on real VM" `Slow (fun () ->
        ...
        Fun.protect ~finally:cleanup (fun () ->
            ...
            let post_launch_dir =
              Filename.concat project_hooks_dir "post-launch.d"
            in
            ...
            let guest_init_dir =
              Filename.concat project_hooks_dir "guest-init.d"
            in
            ...
            in
            let hooks =
              Epi.Hooks.discover ~instance_name ~nix_hooks "post-launch"
            in
            Alcotest.(check bool)
              "post-launch has file + nix hooks" true
              (List.length hooks >= 2);
            ...
            in
            let hooks_pre =
              Epi.Hooks.discover ~instance_name ~nix_hooks:nix_hooks_pre
                "pre-stop"
            in
            Alcotest.(check bool)
              "pre-stop has file + nix hooks" true
              (List.length hooks_pre >= 2);
            ...
            Alcotest.(check (list string))
              ...
              [ "project-post-launch"; "project-pre-stop" ]
              lines));
  ]
