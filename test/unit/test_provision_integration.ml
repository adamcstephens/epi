...
let with_test_dirs f =
  with_temp_dir "epi-prov-int" (fun dir ->
      ...
      with_env "EPI_STATE_DIR" state_dir (fun () ->
          with_env "EPI_CACHE_DIR" cache_dir (fun () -> f ~state_dir ~cache_dir)))
...
let make_descriptor dir =
  ...
  write disk "disk";
  write initrd "initrd";
  { Mock_modules.default_descriptor with kernel; disk; initrd = Some initrd }
    ...
    Alcotest.test_case "successful provision writes state" `Quick (fun () ->
        with_test_dirs (fun ~state_dir ~cache_dir:_ ->
            with_temp_dir "epi-prov-desc" (fun dir ->
                ...
                let runtime =
                  {
                    Mock_modules.default_runtime with
                ...
                in
                let resolver =
                  Mock_modules.make_mock_resolver
                    ~resolve:(fun _target -> Ok descriptor)
                    ()
                in
                let runner =
                  Mock_modules.make_mock_runner
                    ~launch:(fun
                        ~mount_paths:_
                        ~disk_size:_
                        ~instance_name:_
                ...
                in
                match
                  Vm_launch.provision ~resolver ~runner ~rebuild:false
                ...
                | Ok result ->
                    ...
                    Alcotest.(check bool) "state persisted" true (loaded <> None);
                    (match loaded with
                    | Some r ->
                        Alcotest.(check string)
                          "unit_id matches" result.unit_id r.unit_id
                    | None -> ());
                    let instance_dir = Filename.concat state_dir "prov-ok" in
                    Alcotest.(check bool)
                      "instance dir exists" true
            ...
            with_temp_dir "epi-prov-desc" (fun dir ->
                ...
                match
                  Vm_launch.provision ~resolver ~runner ~rebuild:false
                    ...
                    ~target:".#prov-fail" ()
                with
                | Ok _ -> Alcotest.fail "expected provision to fail"
                | Error _ ->
                    let loaded = Instance_store.find_runtime "prov-fail" in
                    Alcotest.(check bool)
                      "no runtime persisted" true (loaded = None))));
                      ...
                      (Some
                         (fun ~rebuild target ->
                           ...
                           let cache_path =
                             ...
                             in
                             if not (Sys.file_exists target_cache) then
                               Unix.mkdir target_cache 0o755;
                             Filename.concat target_cache (hash ^ ".descriptor")
                           in
                           if rebuild && Sys.file_exists cache_path then
                             Sys.remove cache_path;
                           if Sys.file_exists cache_path then
                             Ok (Target.Cached descriptor)
                           else (
                             increment ();
                             Yojson.Basic.to_file cache_path
                ...
                Alcotest.(check bool)
                  ...
                  (List.exists
                     (fun n ->
                       ...
                       len > 11 && String.sub n (len - 11) 11 = ".descriptor")
                     files))));
    Alcotest.test_case "--rebuild forces re-eval" `Quick (fun () ->
        with_test_dirs (fun ~state_dir:_ ~cache_dir ->
            with_temp_dir "epi-prov-desc" (fun dir ->
                ...
                (match
                   Vm_launch.provision ~resolver ~runner ~rebuild:true
