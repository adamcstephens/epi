...
let make_descriptor ~dir =
  ...
  let oc = open_out kernel in output_string oc "kernel"; close_out oc;
  let oc = open_out disk in output_string oc "disk"; close_out oc;
  ...
  [
    Alcotest.test_case "cache write after resolve" `Quick (fun () ->
        with_temp_dir "epi-cache-unit" (fun dir ->
            ...
            let json = Yojson.Basic.pretty_to_string (Target.descriptor_to_json descriptor) in
            ...
            with_env "EPI_CACHE_DIR" cache_dir (fun () ->
              with_env "EPI_TARGET_RESOLVER_CMD" resolver_script (fun () ->
                match Target.resolve_descriptor_cached ~rebuild:false ".#test-cache" with
                | Error e -> Alcotest.fail e.details
                | Ok (Target.Cached _) -> Alcotest.fail "expected Resolved, got Cached"
                | Ok (Target.Resolved d) ->
                    Alcotest.(check string) "kernel" descriptor.kernel d.kernel;
                    let targets_dir = Filename.concat cache_dir "targets" in
                    let files = Sys.readdir targets_dir |> Array.to_list in
                    let has_descriptor =
                      List.exists (fun name ->
                        let len = String.length name in
                        len > 11 && String.sub name (len - 11) 11 = ".descriptor")
                        files
                    in
                    Alcotest.(check bool) "cache file written" true has_descriptor))));
        ...
        with_temp_dir "epi-cache-hit" (fun dir ->
            ...
            with_env "EPI_CACHE_DIR" cache_dir (fun () ->
              with_env "EPI_TARGET_RESOLVER_CMD" resolver_script (fun () ->
                ...
                let _ = Target.resolve_descriptor_cached ~rebuild:false ".#test-hit" in
                (* Second call: should hit cache *)
                match Target.resolve_descriptor_cached ~rebuild:false ".#test-hit" with
                | Error e -> Alcotest.fail e.details
                | Ok (Target.Resolved _) -> Alcotest.fail "expected Cached on second call"
                | Ok (Target.Cached d) ->
                    Alcotest.(check string) "kernel" descriptor.kernel d.kernel;
                    let count =
    ...
    Alcotest.test_case "cache miss on missing paths triggers re-resolve" `Quick (fun () ->
        with_temp_dir "epi-cache-miss" (fun dir ->
            ...
            with_env "EPI_CACHE_DIR" cache_dir (fun () ->
              with_env "EPI_TARGET_RESOLVER_CMD" resolver_script (fun () ->
                (* First call: resolves and caches *)
                let _ = Target.resolve_descriptor_cached ~rebuild:false ".#test-miss" in
                  ...
                  let corrupted = match json with
                    | `Assoc fields ->
                        `Assoc (List.map (fun (k, v) ->
                          if String.equal k "disk" then (k, `String "/nonexistent")
                          else (k, v)) fields)
                    | other -> other
                  in
                  Yojson.Basic.to_file path corrupted) files;
                (* Second call: should re-resolve *)
                match Target.resolve_descriptor_cached ~rebuild:false ".#test-miss" with
                | Error e -> Alcotest.fail e.details
                | Ok (Target.Cached _) -> Alcotest.fail "expected Resolved after cache corruption"
    ...
    Alcotest.test_case "--rebuild busts cache" `Quick (fun () ->
        with_temp_dir "epi-cache-rebuild" (fun dir ->
            ...
            with_env "EPI_CACHE_DIR" cache_dir (fun () ->
              with_env "EPI_TARGET_RESOLVER_CMD" resolver_script (fun () ->
                (* First call: resolves and caches *)
                let _ = Target.resolve_descriptor_cached ~rebuild:false ".#test-rebuild" in
                (* Second call with rebuild: should re-resolve *)
                match Target.resolve_descriptor_cached ~rebuild:true ".#test-rebuild" with
                | Error e -> Alcotest.fail e.details
                | Ok (Target.Cached _) -> Alcotest.fail "expected Resolved after rebuild"
                | Ok (Target.Resolved _) ->
                    let count =
                      if Sys.file_exists resolver_log then
                        In_channel.with_open_text resolver_log In_channel.input_all
