...
let tests =
  [
            ...
            in_dir dir (fun () ->
                ...
                | Ok c ->
                    Alcotest.(check (option string))
                      "target" (Some ".#dev") c.Config.target;
                    Alcotest.(check (option (list string)))
                      "mounts" (Some [ "/home/user/src" ]) c.Config.mounts;
                    Alcotest.(check (option string))
                      "disk_size" (Some "80G") c.Config.disk_size)));
    Alcotest.test_case "missing file returns empty" `Quick (fun () ->
        with_temp_dir "epi-config-miss" (fun dir ->
            in_dir dir (fun () ->
                ...
                | Ok c ->
                    Alcotest.(check (option string))
                      "target" None c.Config.target;
                    Alcotest.(check (option (list string)))
                      "mounts" None c.Config.mounts;
                    Alcotest.(check (option string))
                      "disk_size" None c.Config.disk_size)));
        ...
        with_temp_dir "epi-config-bad" (fun dir ->
            ...
            in_dir dir (fun () ->
                ...
                | Error msg ->
                    Alcotest.(check bool)
                      "contains path" true
                      (Util.contains msg ".epi/config.toml"))));
            ...
            in_dir dir (fun () ->
                ...
                | Ok c ->
                    ...
                    Alcotest.(check (option (list string)))
                      "mounts" None c.Config.mounts;
                    Alcotest.(check (option string))
                      "disk_size" (Some "50G") c.Config.disk_size)));
    Alcotest.test_case "config mount paths resolve against project root" `Quick
      (fun () ->
        with_temp_dir "epi-config-mounts" (fun dir ->
            ...
            in_dir dir (fun () ->
                ...
                | Ok c ->
                    let expected =
                      [ Filename.concat dir "src"; Filename.concat dir "data" ]
                    in
                    Alcotest.(check (option (list string)))
                      "mounts" (Some expected) c.Config.mounts)));
    ...
    Alcotest.test_case "CLI resolve_path relative resolves against base" `Quick
      (fun () ->
    ...
    Alcotest.test_case "merge: config values used when CLI args absent" `Quick
      (fun () ->
        let config : Config.t =
          {
            target = Some ".#dev";
            mounts = Some [ "/mnt/data" ];
            disk_size = Some "80G";
          }
        in
        match
          Config.merge ~cli_target:None ~cli_mounts:[] ~cli_disk_size:None
            config
        with
        | Error e -> Alcotest.fail e
        | Ok r ->
            ...
            Alcotest.(check (list string))
              "mounts" [ "/mnt/data" ] r.Config.resolved_mounts;
            Alcotest.(check string)
              "disk_size" "80G" r.Config.resolved_disk_size);
    Alcotest.test_case "merge: CLI args override config values" `Quick
      (fun () ->
        let config : Config.t =
          {
            target = Some ".#dev";
        ...
        in
        match
          Config.merge ~cli_target:(Some ".#prod") ~cli_mounts:[ "/cli/mount" ]
            ~cli_disk_size:(Some "20G") config
        with
        | Error e -> Alcotest.fail e
        | Ok r ->
            Alcotest.(check string) "target" ".#prod" r.Config.resolved_target;
            Alcotest.(check (list string))
              "mounts" [ "/cli/mount" ] r.Config.resolved_mounts;
            Alcotest.(check string)
              "disk_size" "20G" r.Config.resolved_disk_size);
    Alcotest.test_case "merge: no target produces error mentioning config file"
      `Quick (fun () ->
        match
          Config.merge ~cli_target:None ~cli_mounts:[] ~cli_disk_size:None
            Config.empty
        ...
        | Error msg ->
            Alcotest.(check bool)
              "mentions config.toml" true
              (Util.contains msg "config.toml"));
    Alcotest.test_case "user_config_path: EPI_CONFIG_FILE takes precedence"
      `Quick (fun () ->
        let saved_epi = Sys.getenv_opt "EPI_CONFIG_FILE" in
        let saved_xdg = Sys.getenv_opt "XDG_CONFIG_HOME" in
        Fun.protect
          ~finally:(fun () ->
            (match saved_epi with
            | Some v -> Unix.putenv "EPI_CONFIG_FILE" v
            | None -> Unix.putenv "EPI_CONFIG_FILE" "");
            (if saved_epi = None then
               try Unix.putenv "EPI_CONFIG_FILE" "" with _ -> ());
    ...
    Alcotest.test_case
      "user_config_path: XDG_CONFIG_HOME used when no EPI_CONFIG_FILE" `Quick
      (fun () ->
        with_temp_dir "epi-ucp" (fun _dir ->
            ...
            in
            let saved =
              List.map save_env [ "EPI_CONFIG_FILE"; "XDG_CONFIG_HOME" ]
            in
            Fun.protect
              ~finally:(fun () ->
                List.iter
              ...
              (fun () ->
                ...
                | Some p ->
                    Alcotest.(check string)
                      "path" "/xdg/custom/epi/config.toml" p
                | None -> Alcotest.fail "expected Some")));
    Alcotest.test_case "user_config_path: defaults to HOME/.config" `Quick
      (fun () ->
        ...
        with_temp_dir "epi-load-user" (fun dir ->
            ...
            Fun.protect
              ...
              (fun () ->
                ...
                | Ok c ->
                    Alcotest.(check (option string))
                      "target" (Some ".#user-target") c.Config.target)));
    Alcotest.test_case "load_user: error when EPI_CONFIG_FILE set but missing"
      `Quick (fun () ->
        let saved = Sys.getenv_opt "EPI_CONFIG_FILE" in
        Fun.protect
          ~finally:(fun () ->
          ...
          (fun () ->
            ...
            | Error msg ->
                Alcotest.(check bool)
                  "mentions path" true
                  (Util.contains msg "/nonexistent/config.toml")));
    Alcotest.test_case "load_user: empty config when default path missing"
      `Quick (fun () ->
    ...
    Alcotest.test_case "merge_configs: project overrides user" `Quick (fun () ->
        ...
        in
        let project : Config.t =
          { target = Some ".#proj"; mounts = None; disk_size = Some "60G" }
        in
        let merged = Config.merge_configs ~user ~project in
        Alcotest.(check (option string))
          "target" (Some ".#proj") merged.Config.target;
        Alcotest.(check (option (list string)))
          "mounts" (Some [ "/user/m" ]) merged.Config.mounts;
        Alcotest.(check (option string))
          "disk_size" (Some "60G") merged.Config.disk_size);
    Alcotest.test_case "three-tier: CLI > project > user" `Quick (fun () ->
        ...
        in
        let project : Config.t =
          { target = Some ".#proj"; mounts = None; disk_size = None }
        in
        let merged = Config.merge_configs ~user ~project in
        match
          Config.merge ~cli_target:(Some ".#cli") ~cli_mounts:[]
            ~cli_disk_size:None merged
        with
        | Error e -> Alcotest.fail e
        | Ok r ->
            Alcotest.(check string)
              "target from CLI" ".#cli" r.Config.resolved_target;
            Alcotest.(check (list string))
              "mounts from user" [ "/user/m" ] r.Config.resolved_mounts;
            Alcotest.(check string)
              "disk_size from user" "20G" r.Config.resolved_disk_size);
  ]
