...
let tests =
  [
    Alcotest.test_case "alloc_free_port returns valid port" `Quick (fun () ->
        ...
        Alcotest.(check bool) "port in range"
          true (port >= 1 && port <= 65535));
    Alcotest.test_case "alloc_free_port returns distinct ports" `Quick (fun () ->
        let p1 = Vm_launch.alloc_free_port () in
        let p2 = Vm_launch.alloc_free_port () in
        Alcotest.(check bool) "ports differ" true (p1 <> p2));
    Alcotest.test_case "ensure_writable_disk copies nix-store disk" `Quick (fun () ->
            ...
            with_env "EPI_STATE_DIR" state_dir (fun () ->
              ...
              with_env "EPI_QEMU_IMG_BIN" "true" (fun () ->
                ...
                match Vm_launch.ensure_writable_disk
                        ~instance_name:"disk-test" ~target:".#test"
                        ~disk_size:"40G" descriptor with
                | Ok path ->
                    (* Non-store path returns original disk *)
                    Alcotest.(check string) "returns original disk"
                      nix_store_disk path
                | Error err ->
                    Alcotest.fail (Vm_launch.pp_provision_error err)))));
    Alcotest.test_case "ensure_writable_disk returns original for non-store path" `Quick (fun () ->
