...
- NixOS module: Build the disk image at a fixed 20G size instead of `Minimize = "guess"`, which populated the filesystem twice to find the minimal size — image builds are roughly twice as fast. The intermediate raw image is sparse on disk; zstd-compressed qcow2 keeps the distributed image compact through `nix copy` and binary caches.
