...
The service SHALL read `user.ssh_authorized_keys` from `epi.json` and write them to `/home/<username>/.ssh/authorized_keys`, creating the `.ssh` directory if needed. The directory SHALL have permissions 700 and the file SHALL have permissions 600, both owned by the user.

#### Scenario: SSH keys written for new user
- **WHEN** epi-init creates a new user and `epi.json` contains two SSH keys
- **THEN** `/home/<username>/.ssh/authorized_keys` contains both keys
- **AND** `.ssh` has permissions 700 and `authorized_keys` has permissions 600

#### Scenario: SSH keys updated for existing user
- **WHEN** epi-init runs and the user already exists
- **THEN** `/home/<username>/.ssh/authorized_keys` is overwritten with keys from `epi.json`

#### Scenario: No SSH keys in epi.json
- **WHEN** `epi.json` has no `ssh_authorized_keys` field or it is empty
- **THEN** no `.ssh/authorized_keys` file is created or modified

### Requirement: epi-init sets hostname
The service SHALL set the system hostname to the value of `hostname` from `epi.json` using `hostnamectl set-hostname`.
