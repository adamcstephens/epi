...
When launching a VM, the CLI SHALL allocate a free host TCP port and start pasta with `-T <host-port>:22` so that host→VM SSH connections are possible via `localhost:<host-port>`.
