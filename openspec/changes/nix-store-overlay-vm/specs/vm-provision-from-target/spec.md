...
- **THEN** the script MUST output a JSON object with keys: `kernel`, `disk`, `initrd`, `cmdline`, `cpus`, `memory_mib`, `configuredUsers`, `overlayStore`
- **AND** the CLI SHALL parse the output as JSON using the same parser as `nix eval --json`

#### Scenario: Guest hooks are embedded in seed ISO during provisioning
...
#### Scenario: Descriptor includes overlayStore field
- **WHEN** the target is resolved
- **THEN** the descriptor contains an `overlayStore` boolean field
- **AND** when `overlayStore` is `true`, the `disk` path references a minimal image
- **AND** when `overlayStore` is `false`, the `disk` path references the traditional full image
