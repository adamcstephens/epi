...
The CLI SHALL provide `epi console [INSTANCE]` to attach interactively to the running instance serial console, defaulting to `default` when omitted.

#### Scenario: Attach to explicit instance
- **WHEN** a user runs `epi console dev-a` and `dev-a` is running with a valid serial endpoint
- **THEN** the CLI connects to the serial console for `dev-a`
...
- **THEN** the CLI connects to the serial console for `default`
