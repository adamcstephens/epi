## Purpose
Define how console output is captured to a persistent file via cloud-hypervisor's virtio-console device.
...
- **WHEN** a VM boots with cmdline containing `console=ttyS0 console=hvc0`
- **THEN** the virtio-console file captures kernel and systemd boot messages
- **AND** the serial socket also receives kernel printk messages
