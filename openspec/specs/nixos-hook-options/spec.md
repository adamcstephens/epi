...
#### Scenario: No host hooks yields empty arrays
- **WHEN** both `epi.hooks.post-launch` and `epi.hooks.pre-stop` are empty
- **THEN** the target descriptor contains a `hooks` field with empty `post-launch` and `pre-stop` arrays
