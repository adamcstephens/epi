## Purpose
Define the `start` subcommand that resumes an existing stopped VM instance without requiring a `--target` flag.
...
The `start` command SHALL generate an SSH keypair and wait for SSH connectivity by default, using the same logic as `launch`. The `--no-provision` flag SHALL skip post-launch provisioning (SSH wait, host key trust, hooks). The `--wait-timeout` flag SHALL be accepted.
...
#### Scenario: Start with --no-provision skips post-launch provisioning
- **WHEN** a user runs `epi start dev-a --no-provision`
